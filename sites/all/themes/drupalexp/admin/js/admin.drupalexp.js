...
(function ($) {
    ...
    function drupalexp_loadLayouts() {
        ...
        $(dexp_layouts).each(function (index) {
            ...
            var copyLayout = $('<span title="Duplicate" class="fa fa-copy"></span>').css({
                color: '#0074BD',
                cursor: 'pointer',
                marginRight: '5px'
            });
            ...
            var editLayout = $('<span title="Rename" class="layout-settings fa fa-edit"></span>').css({
                    color: '#0074BD',
                    cursor: 'pointer',
                    marginRight: '5px'
                });
                editLayout.click(function () {
                    ...
                    var isdefault = dexp_layouts[dexp_current_layout].isdefault||false;
                    $('input[name=layout_default]').prop('checked',isdefault);
                });
                layoutTab.append(editLayout);
            if (this.isdefault == false) {
                var layoutRemove = $('<span title="Delete" class="fa fa-ban"></span>').css({
                    color: '#FF0000',
                    cursor: 'pointer',
                    marginRight: '5px'
                }).click(function () {
    ...
    function drupalexp_saveLayout(layoutIndex) {
        var layout = {
            ...
            isdefault: dexp_layouts[layoutIndex].isdefault||false
        };
    ...
    $(document).ready(function () {
        ...
        $(dexp_layouts).each(function(index){
            dexp_layouts[index].isdefault = dexp_layouts[index].isdefault||false;
            if(dexp_layouts[index].isdefault){
                default_layout = index;
            }
        });
        dexp_layouts[default_layout].isdefault = true;
        drupalexp_loadLayouts();
        drupalexp_loadLayout(0);
        $('li.dexp_layout:first').addClass('active');
        $('#edit-drupalexp-layouts-edit').dialog({
            ...
            buttons: {
                Save: function () {
                    ...
                    */
                    var isdefault = $('input[name=layout_default]:checked').length == 1? true: false;
                    if(isdefault){
                        $(dexp_layouts).each(function(index, layout){
                            dexp_layouts[index].isdefault = false;
                        });
                        dexp_layouts[dexp_current_layout].isdefault = true;
                    }
        ...
        $('form#system-theme-settings').submit(function () {
            ...
            var layoutsstrs = layoutsstr.match(/.{1,1000}/g);
            for (var i = 0; i < layoutsstrs.length; i++) {
                var $input = $('<input type="hidden" name="dexp_layout_' + i + '"/>');
                $input.val(layoutsstrs[i]);
